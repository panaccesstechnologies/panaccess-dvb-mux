...
// Stream is the Phase 2 transport identity used by routing and mux layers.
type Stream struct {
	ID       string
	Source   string
...
func NewStream(id, source, protocol string, queueCapacity int, bitrate uint64) (*Stream, error) {
	...
	return &Stream{
		...
		Queue:    q,
		Pacer:    p,
		Monitor:  NewTSMonitor(),
		RTPSeq:   NewRTPSequenceTracker(),
	}, nil
}
...
func (s *Stream) EnqueueDatagram(datagram []byte) error {
	...
	if err := ValidateMPEGTSRTP(rtp); err != nil {
		s.Stats.AddPacketErrors(1)
		return err
	}
	gap, duplicate := s.RTPSeq.Observe(rtp.SequenceNumber)
	if gap > 0 {
		s.Stats.AddRTPGaps(uint64(gap))
	}
	if duplicate {
		s.Stats.AddRTPGaps(1)
	}
