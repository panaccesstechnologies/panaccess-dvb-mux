...
func TestParseDatagram1316(t *testing.T) {
	...
	if packets[6].PID() != 0x0601 {
		t.Fatalf("unexpected PID 0x%x", packets[6].PID())
	}
}
...
func TestParseRTPMPEGTS(t *testing.T) {
	...
	if err := ValidateMPEGTSRTP(p); err != nil {
		t.Fatal(err)
	}
	if p.PayloadType != 33 || p.SequenceNumber != 42 || p.SSRC != 0x12345678 {
		t.Fatalf("bad RTP header: %+v", p)
	}
}

func TestRTPSequenceTracker(t *testing.T) {
	var tr RTPSequenceTracker
	if gap, dup := tr.Observe(100); gap != 0 || dup {
		t.Fatal("first packet should sync")
	}
	if gap, dup := tr.Observe(101); gap != 0 || dup {
		t.Fatal("contiguous packet reported incorrectly")
	}
	if gap, dup := tr.Observe(101); !dup || gap != 0 {
		t.Fatal("duplicate not detected")
	}
	if gap, dup := tr.Observe(104); gap != 2 || dup {
		t.Fatalf("expected gap 2, got %d duplicate=%v", gap, dup)
	}
}
...
func TestBatch(t *testing.T) {
	...
	for i := range packets {
		packets[i].Data[0] = mpegts.SyncByte
	}
	...
	if n != 7 || len(b) != 7*188 {
		t.Fatalf("got %d packets / %d bytes", n, len(b))
	}
}
...
func TestPacerDuration(t *testing.T) {
	...
	if got := p.DelayForBytes(1000); got != 1*time.Millisecond {
		t.Fatalf("got %v", got)
	}
}
