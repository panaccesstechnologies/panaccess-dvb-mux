...
func TestPATRoundTrip(t *testing.T) {
	...
	s, err := in.MarshalSection(); if err != nil { t.Fatal(err) }
	out, err := ParsePAT(s); if err != nil { t.Fatal(err) }
...
func TestPMTRoundTrip(t *testing.T) {
	...
	s, err := in.MarshalSection(); if err != nil { t.Fatal(err) }
	out, err := ParsePMT(s); if err != nil { t.Fatal(err) }
...
func TestPacketizeSection(t *testing.T) {
	...
	pkts, err := PacketizeSection(0x100, s, 14); if err != nil { t.Fatal(err) }
...
func TestPCRRoundTrip(t *testing.T) {
	...
	got, err := DecodePCR(a); if err != nil { t.Fatal(err) }
